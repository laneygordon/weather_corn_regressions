...
Adding Tmax^2 to the model is helpful because it allows us to see normal distributed from temperatures 24 degrees C to about 31 degrees C. The summary statistics of the data also implies there is significant evidence (because the p-value is less than 0.05 alpha) that temperatures between 26 and 28 degrees C have the highest yield.   
...
```{r}
cross_section18 <- inner_join(cornyields, tmaxdf) %>%
                 filter(year== 2018 & !is.na(tmax)) %>% 
                group_by(countyfp, year, yield) %>%
                summarize(meantmax = mean(tmax) ) 
