...
Adding Tmax^2 to the model is beneficial to the data because it helps us identify the correlation between temperature and yield. The graph displays normal distribution from temperatures 24 degrees C to about 31 degrees C and shows the highest yield occurs from temperatures 26 to 28 degrees C. The p-value of 0.0078 is greater than alpha (0.05), thus we have significant evidence of a correlation.  
...
#linear model
lm_Q3 <- lm(yield ~ meantmax, data = county_summer_yield) 
summary(lm_Q3) 

#plot
ggplot(county_summer_yield, aes( x = meantmax, y= yield)) +
   geom_point() + 
  theme_bw() +
  labs(x = "Max Temperature (C)", y = "Yield")   
```
Based on the plot, there does not appear to be a relationship between the meantmax and yield, and the p-value of 0.063 is greater than alpha which assures us that we have enough evidence to support this conclusion.  
